...
class Ventana3(Frame):
    ...
    def llenarDatos(self):
        datos = self.docentes.consulta_docente()
        for row in datos:
    ...
    def fEliminar(self):
        ...
        if clave == '':
            ...
            if r == messagebox.YES:
                n = self.docentes.elimina_docente(clave)
                if n == 1:
    ...
    def fGuardar(self):
        if self.id == -1:
            self.docentes.inserta_docente(self.textNombre.get(), self.textEdad.get(), self.textSueldo.get())
            messagebox.showinfo("Insertar", 'Elemento Insertado')
        else:
            self.docentes.modifica_docente(self.id, self.textNombre.get(), self.textEdad.get(),
                                           self.textSueldo.get())
    ...
    def create_widgets(self):
        ...
        self.textSueldo.place(x=3, y=175, width=100, height=20)
